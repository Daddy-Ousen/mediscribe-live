...
export const AudioWaveform: React.FC<AudioWaveformProps> = ({
  ...
  return (
    <div className="bg-obsidian-300 border border-console-border rounded-xl p-4 font-mono text-xs">
      <div className="flex items-center justify-between border-b border-console-border pb-2.5 mb-3 text-[11px] text-slate-400">
          ...
          {Array.from({ length: segments }).map((_, idx) => {
            ...
            if (isActive) {
              if (isRedZone) {
                ...
                color = isSpeaking ? 'bg-cyan-400' : 'bg-emerald-400';
              }
            }
      ...
      <div className="grid grid-cols-3 gap-2 mt-3 pt-2.5 border-t border-console-border/50 text-[10px]">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-slate-400">NEURAL VAD:</span>
          <span className="text-slate-200 font-semibold">{isListening ? 'TRIGGERED' : 'QUIET'}</span>
        </div>

        <div className="flex items-center gap-1.5 justify-end">
          <span className={`w-1.5 h-1.5 rounded-full ${
            isInterrupted ? 'bg-amber-400' : isSpeaking ? 'bg-cyan-400' : 'bg-slate-600'
          }`} />
